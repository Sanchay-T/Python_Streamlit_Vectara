...
# Adding an interactive element, like a button, to engage users right from the start
...
with st.sidebar:
    ...
    st.session_state["serper_api_key"] = st.text_input("Serper API Key")
    vectara_customer_id = st.text_input("Vectara Customer ID")
    corpus_name = st.text_input("Corpus Name")
    corpus_description = st.text_input("Corpus Description")
    file = st.file_uploader("Upload a file", type=["text", "pdf"])
...
# Process user prompts and update the corpus
